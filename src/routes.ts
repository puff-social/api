...
export function InternalRoutes(server: FastifyInstance, opts: FastifyPluginOptions, next: () => void) {
  server.get('/verify', async (req, res) => {
    ...
    return res.status(200).send({ valid: true, user: sanitize(user, ['platform', 'platform_id', 'refresh_token']) });
  });

  next();
}

export function AuthedRoutes(server: FastifyInstance, opts: FastifyPluginOptions, next: () => void) {
  server.register(AuthMiddleware, { required: true });

  server.get('/user', async (req, res) => {
    return res.status(200).send({ success: true, data: { user: sanitize(req.user, ['platform', 'platform_id', 'refresh_token']) } });
  });
...
export function Routes(server: FastifyInstance, opts: FastifyPluginOptions, next: () => void) {
  ...
  server.get('/oauth/:platform', async (req: FastifyRequest<{ Params: { platform: string } }>, res) => {
    switch (req.params.platform) {
      case 'discord': {
        const state = pika.gen('oauth');
        await keydb.set(`oauth_state/${state}`, state, 'EX', 120);
        const params = stringify({ client_id: env.DISCORD_CLIENT_ID, response_type: 'code', scope: 'identify', state, redirect_uri: `${env.APPLICATION_HOST}/callback/discord` });
        return res.status(200).send({ success: true, data: { url: `https://discord.com/oauth2/authorize?${params}` } });
      }
      default: {
        return res.status(400).send({ success: false, error: 'invalid_platform' });
      }
    }
  });

  server.post('/oauth/:platform', async (req: FastifyRequest<{ Params: { platform: string }, Querystring: { state: string; code: string } }>, res) => {
    const { state, code } = req.query;

    switch (req.params.platform) {
      case 'discord': {
        const validState = await keydb.exists(`oauth_state/${state}`);
        if (!validState) return res.status(400).send({ success: false, error: 'invalid_state' });

        const tokens = await exchangeDiscordCode(code);
        const user = await fetchDiscordUser(tokens.access_token);

        await keydb.set(`oauth/discord/${user.id}`, tokens.access_token, 'EX', tokens.expires_in);
        await keydb.del(`oauth_state/${state}`);

        const existingUser = await prisma.users.findFirst({ where: { platform_id: user.id } });
        if (existingUser) {
        ...
        return res.status(200).send({ success: true, data: { user: { id, name: user.username, image: hash }, token: session } });
      }
      default: {
        return res.status(400).send({ success: false, error: 'invalid_platform' });
      }
