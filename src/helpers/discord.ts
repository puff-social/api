...
export async function exchangeDiscordCode(code: string) {
  const req = await fetch('https://discord.com/api/oauth2/token', {
    ...
    body: stringify({ code, grant_type: 'authorization_code', redirect_uri: `${env.APPLICATION_HOST}/callback/discord`, client_id: env.DISCORD_CLIENT_ID, client_secret: env.DISCORD_CLIENT_SECRET })
  });
